    ...
    organizationName: z
      ...
      .refine(
        (organizationName) => {
          ...
          const hasWhiteSpace = regex.test(organizationName);
      ...
      .refine(
        ...
        (organizationName) => {
           ...
           **/
          const { data: result = [] } = organizationApiSlice.endpoints.fetchOrgList.select(undefined)(store.getState());

          if (result.some((org) => org.name?.toLowerCase() === organizationName?.toLowerCase())) {
            return false;
          }
